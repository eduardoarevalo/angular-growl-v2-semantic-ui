      ...
      controller: [     '$scope',      '$timeout',      'growl',      'growlMessages',
        function ($scope, $timeout, growl, growlMessages) 
        {
          ...
          $scope.alertClasses = function (message) {
            return {
              'green': message.severity === 'success',
              'red': message.severity === 'error',
              'blue': message.severity === 'info',
              'yellow' : message.severity === 'warning',
            };
          };

           $scope.iconClasses = function (message) {
            return {
              'checkmark': message.severity === 'success',
              'remove circle': message.severity === 'error',
              'info margen-izquierdo': message.severity === 'info',
              'warning sign': message.severity === 'warning',
              'small icon align-right ': message.disableIcons === false,
            };
          };
  ...
  function ($templateCache) {
    ...
    if ($templateCache.get('templates/growl/growl.html') === undefined) {
      ...
      '<div class="growl-container" ng-class="wrapperClasses()">' + 
        '<div class="ui icon message growl-item" ng-repeat="message in growlMessages.directives[referenceId].messages" ng-class="alertClasses(message)" >'+ 
          '<i ng-class="iconClasses(message)" aria-hidden="true"></i>' + 
          '<i class="close icon" ng-click="growlMessages.deleteMessage(message)" ng-show="!message.disableCloseButton"></i>'+
          '<div aria-hidden="true" class="floating ui label" ng-class="alertClasses(message)" ng-show="showCountDown(message)">{{message.countdown}}s</div>'+
          '<div class="content">'+
            '<div class="header" ng-show="message.title" ng-bind="message.title"></div>'+
